    ...
    <section id="services" className="py-24 bg-tactical-black border-y border-military/10">
      <div className="container mx-auto px-4 md:px-6">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-4">
          ...
          <p className="max-w-md text-gray-200 text-lg">
            ...
            <div 
              ...
              className={`p-8 rounded-sm border transition-all duration-300 group ${
                service.primary 
                  ...
                  : 'bg-tactical-gray border-white/10 text-white hover:border-military/50'
              }`}
            >
              <div className={`mb-6 inline-block p-4 rounded-sm ${service.primary ? 'bg-black text-military' : 'bg-military/20 text-military'}`}>
                {service.icon}
              </div>
              <h3 className="text-2xl font-display font-black uppercase mb-4 tracking-tight">
                {service.title}
              </h3>
              <p className={`text-lg leading-relaxed ${service.primary ? 'text-black/90 font-medium' : 'text-gray-200'}`}>
                {service.description}
              </p>
              {service.primary && (
                <div className="mt-6 pt-6 border-t border-black/10 text-sm font-black uppercase tracking-widest">
